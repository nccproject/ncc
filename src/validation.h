...
#include <ncc/storageresults.h>


extern std::unique_ptr<NCCState> globalState;
...
using ExtractNCCTX = std::pair<std::vector<NCCTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const int64_t DEFAULT_MAX_TIP_AGE = 72 * 60 * 60; //Changed to 72 hours so that isInitialBlockDownload() is more accurate
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // ncc
...
inline int64_t FutureDrift(uint32_t nTime) { return nTime + 10; }
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashNCCRoot,
    const dev::h256& oldHashStateRoot, const std::vector<NCCTransaction>& transactions);
...
class NCCTxConverter{

public:

    NCCTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags){}

    bool extractionNCCTransactions(ExtractNCCTX& nccTx);
...
private:
    ...
    NCCTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<NCCTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
