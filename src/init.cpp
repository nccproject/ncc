 ...
 */
static const char* BITCOIN_PID_FILENAME = "nccd.pid";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateNCC");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                dev::eth::Ethash::init();
                fs::path nccStateDir = GetDataDir() / "stateNCC";
                bool fStatus = fs::exists(nccStateDir);
                const std::string dirNCC(nccStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsNCCstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<NCCState>(new NCCState(dev::u256(0), NCCState::openDB(dirNCC, hashDB, dev::WithExisting::Trust), dirNCC, existsNCCstate));
                dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::nccMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(nccStateDir.string()));
                if (fReset) {
