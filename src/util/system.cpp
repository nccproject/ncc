...
const char * const BITCOIN_CONF_FILENAME = "ncc.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NCC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NCC
    // Mac: ~/Library/Application Support/NCC
    // Unix: ~/.ncc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NCC";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NCC";
#else
    // Unix
    return pathRet / ".ncc";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("NCC Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The NCC Core developers";
    }
