...
bool ToHash160(const std::string& strNCCAddress, std::string& strHash160)
{
    CTxDestination nccAddress = DecodeDestination(strNCCAddress);
    if(!IsValidDestination(nccAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&nccAddress);
    if(keyid){
...
bool ToNCCAddress(const std::string& strHash160, std::string& strNCCAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination nccAddress = keyid;
    if(IsValidDestination(nccAddress)){
        strNCCAddress = EncodeDestination(nccAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToNCCAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToNCCAddress(tokenEvent.receiver, tokenEvent.receiver);
