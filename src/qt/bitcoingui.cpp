...
#include <qt/nccversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    nccVersionChecker = new NCCVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a NCC address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ncc: URIs)"));
    ...
    historyAction = new QAction(platformStyle->MultiStatesIcon(":/icons/history"), tr("&Transactions"), this);//NCC_LINE
    ...
    NCCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(NCCTokenAction);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your NCC addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified NCC addresses"));
    ...
    openAction->setStatusTip(tr("Open a ncc: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible NCC command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->mapGroup(NCCTokenAction, tokenActions);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && nccVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(NCC_RELEASES, NCC_RELEASES);
            QString message(tr("New version of NCC wallet is available on the NCC source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    NCCTokenAction->setEnabled(enabled);
...
void BitcoinGUI::gotoSendTokenPage()
{
    ...
    NCCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to NCC network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("NCC"); // default title
