#include <qt/ncctoken.h>
#include <qt/forms/ui_ncctoken.h>
...
NCCToken::NCCToken(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::NCCToken),
    m_model(0),
    m_clientModel(0),
    ...
    m_tokenTransactionView(0)
{
    ...
    connect(copyTokenAddressAction, &QAction::triggered, this, &NCCToken::copyTokenAddress);
    connect(copyTokenBalanceAction, &QAction::triggered, this, &NCCToken::copyTokenBalance);
    connect(copyTokenNameAction, &QAction::triggered, this, &NCCToken::copyTokenName);
    connect(copySenderAction, &QAction::triggered, this, &NCCToken::copySenderAddress);
    connect(removeTokenAction, &QAction::triggered, this, &NCCToken::removeToken);

    connect(ui->tokensList, &QListView::clicked, this, &NCCToken::on_currentTokenChanged);
    connect(ui->tokensList, &QListView::customContextMenuRequested, this, &NCCToken::contextualMenu);
...
void NCCToken::setModel(WalletModel *_model)
{
    m_model = _model;
    ...
    if(m_model && m_model->getTokenItemModel())
    {
        ...
        connect(ui->tokensList->selectionModel(), &QItemSelectionModel::currentChanged, this, &NCCToken::on_currentChanged);

        // Set current token
        connect(m_tokenModel, &QAbstractItemModel::dataChanged, this, &NCCToken::on_dataChanged);
        connect(m_tokenModel, &QAbstractItemModel::rowsInserted, this, &NCCToken::on_rowsInserted);
...
void NCCToken::setClientModel(ClientModel *_clientModel)
{
...
void NCCToken::on_goToSendTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(0);
}

void NCCToken::on_goToReceiveTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(1);
}

void NCCToken::on_goToAddTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(2);
}

void NCCToken::on_currentTokenChanged(QModelIndex index)
{
    if(m_tokenModel)
    {
        if(index.isValid())
...
void NCCToken::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void NCCToken::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void NCCToken::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void NCCToken::contextualMenu(const QPoint &point)
{
