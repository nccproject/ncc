...
#define paternVersion "ncc-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

NCCVersionChecker::NCCVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

NCCVersionChecker::~NCCVersionChecker()
{

}

bool NCCVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> NCCVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(NCC_RELEASES)));
    QEventLoop event;
...
Version NCCVersionChecker::getMaxReleaseVersion()
{
