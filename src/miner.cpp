...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setNCCSchedule(nccDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = nccDGP.getBlockSize(nHeight);
    minGasPrice = nccDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = nccDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    NCCTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractNCCTX resultConverter;
    if(!convert.extractionNCCTransactions(resultConverter)){
    ...
    for(NCCTransaction nccTransaction : nccTransactions){
        txGas += nccTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + nccTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(nccTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, nccTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "nccstake";
...
void StakeNCCs(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
