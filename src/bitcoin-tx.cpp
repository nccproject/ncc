...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " ncc-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  ncc-tx [options] <hex-tx> [commands]  Update hex-encoded ncc transaction\n" +
            "or:     ncc-tx [options] -create [commands]   Create hex-encoded ncc transaction\n" +
            "\n";
