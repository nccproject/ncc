...
#if defined(NDEBUG)
# error "NCC cannot be compiled without assertions."
#endif
...
std::unique_ptr<NCCState> globalState;
...
const std::string strMessageMagic = "NCC Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = nccDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = nccDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            ...
            NCCTxConverter converter(tx, NULL, NULL, contractflags);
            ExtractNCCTX resultConverter;
            if(!converter.extractionNCCTransactions(resultConverter)){
            ...
            std::vector<NCCTransaction> nccTransactions = resultConverter.first;
            std::vector<EthTransactionParams> nccETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(NCCTransaction nccTransaction : nccTransactions){
                sumGas += nccTransaction.gas() * nccTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, nccTransaction.gasPrice());
                } else {
                    txMinGasPrice = nccTransaction.gasPrice();
                }
                VersionVM v = nccTransaction.getVersion();
                if(v.format!=0)
                ...
                if(nccTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(nccTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(nccTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)nccTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(nccETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > nccTransactions.size())
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s NCC additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // ncc
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // ncc
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    NCCTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<NCCTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path nccDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(nccDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(nccDir.string(), std::ios::in | std::ios::out);
...
bool NCCTxConverter::extractionNCCTransactions(ExtractNCCTX& ncctx){
    ...
    std::vector<NCCTransaction> resultTX;
    ...
    ncctx = std::make_pair(resultTX, resultETP);
    return true;
}

bool NCCTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool NCCTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
NCCTransaction NCCTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    NCCTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = NCCTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = NCCTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t NCCTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // ncc
    NCCDGP nccDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setNCCSchedule(nccDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = nccDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = nccDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = nccDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // ncc
...
///////////////////////////////////////////////////////////////////////////////////////// ncc
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            NCCTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractNCCTX resultConvertNCCTX;
            if(!convert.extractionNCCTransactions(resultConvertNCCTX)){
            ...
            if(!CheckMinGasPrice(resultConvertNCCTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertNCCTX.first, blockGasLimit, pindex->pprev);
            ...
            for(NCCTransaction& qtx : resultConvertNCCTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertNCCTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertNCCTX.first[k].from(), resultConvertNCCTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(11600000000 + ((pindex->nHeight - 5000) * 0)) * COIN){//(uint64_t)(100000000 + ((pindex->nHeight - 5000) * 4)) * COIN){
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // ncc
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // ncc
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // ncc
            globalState->setRootUTXO(oldHashUTXORoot); // ncc
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // ncc
        uint32_t sizeBlockDGP = nccDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for NCC PoS we need to sync this after every block to ensure txdb is populated for
