...
inline void initState(){
    ...
    const std::string dirNCC = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<NCCState>(new NCCState(dev::u256(0), NCCState::openDB(dirNCC, hashDB, dev::WithExisting::Trust), dirNCC + "/nccDB", dev::eth::BaseState::Empty));
...
inline dev::Address createNCCAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline NCCTransaction createNCCTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    NCCTransaction txEth;
    if(recipient == dev::Address()){
        txEth = NCCTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = NCCTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<NCCTransaction> txs){
