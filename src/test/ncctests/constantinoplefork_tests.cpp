...
void genesisLoading(){
    ...
    dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::nccMainNetwork, 999)));
...
BOOST_AUTO_TEST_CASE(checking_returndata_opcode_after_fork){
    ...
    txs.push_back(createNCCTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address()));
    txs.push_back(createNCCTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++HASHTX, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createNCCAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<NCCTransaction> txsCall;
    txsCall.push_back(createNCCTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<NCCTransaction> txsMint;
    txsMint.push_back(createNCCTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<NCCTransaction> txsbalance;
    txsbalance.push_back(createNCCTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
...
BOOST_AUTO_TEST_CASE(checking_constantinople_after_fork){
    ...
    // Create contract
    std::vector<NCCTransaction> txs;
    txs.push_back(createNCCTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createNCCAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<NCCTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createNCCTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
