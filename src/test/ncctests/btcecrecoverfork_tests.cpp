...
void genesisLoading(){
    ...
    dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::nccMainNetwork)));
...
BOOST_AUTO_TEST_CASE(checking_btcecrecover_after_fork){
    ...
    std::vector<NCCTransaction> txBtcEcrecover;
    txBtcEcrecover.push_back(createNCCTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
    txBtcEcrecover.push_back(createNCCTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
    txBtcEcrecover.push_back(createNCCTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
    txBtcEcrecover.push_back(createNCCTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), ++HASHTX, proxy));
