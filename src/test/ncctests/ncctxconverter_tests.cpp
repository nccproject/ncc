...
void checkResult(bool isCreation, std::vector<NCCTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    NCCTxConverter converter(transaction, NULL);
    ExtractNCCTX nccTx;
    BOOST_CHECK(converter.extractionNCCTransactions(nccTx));
    std::vector<NCCTransaction> result = nccTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(ncctxconverter_tests, TestingSetup)
