...
#include <ncc/nccstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

NCCState::NCCState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = NCCState::openDB(_path + "/nccDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

NCCState::NCCState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute NCCState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, NCCTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createNCCAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            ncc::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> NCCState::vins() const // temp
{
...
void NCCState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* NCCState::vin(dev::Address const& _a) const
{
    return const_cast<NCCState*>(this)->vin(_a);
}

Vin* NCCState::vin(dev::Address const& _addr)
{
...
// void NCCState::commit(CommitBehaviour _commitBehaviour)
// {
...
void NCCState::kill(dev::Address _addr)
{
...
void NCCState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void NCCState::deleteAccounts(std::set<dev::Address>& addrs){
...
void NCCState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void NCCState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void NCCState::validateTransfersWithChangeLog(){
