...
void NCCDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool NCCDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule NCCDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t NCCDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t NCCDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t NCCDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t NCCDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool NCCDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void NCCDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void NCCDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void NCCDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void NCCDGP::createParamsInstance(){
...
dev::Address NCCDGP::getAddressForBlock(unsigned int blockHeight){
...
void NCCDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void NCCDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void NCCDGP::parseStorageOneUint64(uint64_t& value){
...
void NCCDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule NCCDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
