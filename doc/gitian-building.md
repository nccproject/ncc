...
*Setup instructions for a Gitian build of NCC Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the NCC
...
- [Building NCC Core](#building-ncc-core)
...
Clone the git repositories for ncc and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/nccproject/ncc --recursive
git clone https://github.com/nccproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/ncc/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the ncc repository under 'Fetch and create inputs' to install sources which require
...
To build NCC Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ncc repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ncc/.git/
...
For example:
```bash
URL=https://github.com/laanwj/ncc.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit ncc=${COMMIT} --url ncc=${URL} ../ncc/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ncc=${COMMIT} --url ncc=${URL} ../ncc/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ncc=${COMMIT} --url ncc=${URL} ../ncc/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-ncc:
./bin/gbuild --commit ncc=${COMMIT},cpp-eth-ncc=${ETHCOMMIT} --url ncc=${URL},cpp-eth-ncc=${ETHURL} ../ncc/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the ncc git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../ncc/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/nccproject/ncc-detached-sigs.git

BTCPATH=/some/root/path/ncc
SIGPATH=/some/root/path/ncc-detached-sigs

./bin/gbuild --url ncc=${BTCPATH},signature=${SIGPATH} ../ncc/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ncc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ncc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/ncc-osx-build.assert
```
...
[nccproject/gitian.sigs](https://github.com/nccproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@ncc.org) and he will commit them.
